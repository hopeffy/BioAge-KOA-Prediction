...
\textbf{Preliminary ELSA replication.}\ The trained model was applied to $n=8{,}416$ respondents from the
English Longitudinal Study of Ageing (ELSA) Wave 8 (2016--2017), using 14 of the 17 raw17 predictors
harmonised from the Gateway Harmonized ELSA file (raw16; Biological Age excluded because ELSA public
nurse visit files do not contain creatinine, BUN, or platelet assays for KDM-BA computation). The KOA
proxy was defined as ever-diagnosed arthritis plus difficulty stooping/kneeling/crouching (prevalence
$24.9\%$). ROC-AUC was $0.604$ (raw) / $0.606$ (isotonic-calibrated), with calibration restored after
split-calibrate-evaluate isotonic recalibration (slope $0.271 \rightarrow 0.938$, ECE $0.097 \rightarrow
0.005$). These results align closely with HRS and confirm a conservative lower bound on raw16
transportability across culturally distinct aging cohorts. A Wave 9 (2018--2019) sensitivity analysis
($n=8{,}683$) showed consistent results (ROC-AUC $0.596$), supporting temporal robustness despite
full BMI imputation in that wave.
\end{abstract}

\textbf{Keywords:} symptomatic KOA, machine learning, TRIPOD, calibration, decision curve analysis,
SHAP, internal validation, external validation, HRS, ELSA, cross-cultural replication
...
% ============================================================
\subsection{Preliminary ELSA Cross-Cultural Replication}
% ============================================================
...
panel of adults aged $\geq 50$, collected by NatCen Social Research, University College London,
and the Institute for Fiscal Studies. For this replication we used the Gateway Harmonized ELSA
file (Version H, 2002--2023), distributed through the UK Data Service (SN 5050). Wave 8 (survey
year 2016--2017) was selected as the primary target wave, as it is the most recent wave with
both complete harmonized variable coverage and nurse visit biomarker data. Wave 9 (2018--2019)
is retained as a temporal sensitivity analysis.

\subsubsection*{ELSA Eligibility and KOA Proxy Definition}
Inclusion criteria: respondents with Wave 8 interview data (\texttt{r8iwstat} indicating completed
interview) and age $\geq 45$ years ($n=8{,}445$ before age filter, $n=8{,}416$ after). Because the
Gateway Harmonized file does not contain a direct symptomatic KOA indicator, a two-component proxy
was constructed:
\begin{enumerate}
  \item \textbf{Ever-diagnosed arthritis:} \texttt{r8arthre} = 1 (``ever diagnosed with
        arthritis or rheumatism''). Unlike HRS, ELSA does not provide a current-wave arthritis
        variable in the harmonized file; the ``ever'' flag is used as the best available proxy.
  \item \textbf{Knee-relevant functional impairment:} \texttt{r8stoopa} = 1 (``any difficulty
        stooping, kneeling, or crouching''), identical to the HRS proxy component.
\end{enumerate}
\[
\text{KOA\_ELSA} = \mathbb{1}\bigl[\texttt{r8arthre}=1\bigr]\ \wedge\
\mathbb{1}\bigl[\texttt{r8stoopa}=1\bigr]
\]
This proxy results in a prevalence of $24.9\%$ ($n_{\text{pos}}=2{,}097$), higher than CHARLS
(13.3\%) but lower than HRS (33.9\%). The use of ``ever-diagnosed'' arthritis (rather than
current-wave) broadens the KOA definition relative to CHARLS; outcome noise from this
definitional difference is documented as a limitation. Missing individual proxy components
were conservatively coded as absence; rows were dropped only when both arthritis and
stooping components were missing, yielding a final analytic sample of $n=8{,}416$.

\subsubsection*{ELSA Feature Harmonisation (raw16)}
Of the 17 raw17 features, 14 could be directly harmonised from the Gateway Harmonized ELSA file
(Table~\ref{tab:elsa-harmonisation}). Three variables were excluded: (1) \emph{Biological Age}
could not be computed from ELSA public nurse visit files because creatinine, BUN/urea, and
platelet assays are not included in the standard ELSA biomarker panel (cholesterol fractions,
HbA1c, CRP, systolic BP, WBC, and hemoglobin are available but insufficient for full KDM-BA);
(2) \emph{Wave identifier} is not applicable across cohorts; and (3) \emph{Survey year} is
set to the target wave. Notably, ELSA raw16 includes \emph{Residence} (from linked rural-urban
indicator data, recently moved to End User Licence) and \emph{Dyslipidemia}
(\texttt{r8hchole}), both of which were missing from HRS raw16. The resulting 14-variable
feature set is also labelled \emph{raw16}. The trained CHARLS model was re-fitted on the
raw16 subset of the CHARLS internal training partition and then applied to ELSA.

\begin{table}[h!]
\centering
\small
\caption{Feature harmonisation map between CHARLS raw17 and ELSA Gateway Harmonized variables
for Wave 8. Entries marked --- indicate variables not available in the ELSA public file.}
\label{tab:elsa-harmonisation}
\begin{tabular}{lllp{5.5cm}}
\toprule
raw17 variable & CHARLS source & ELSA variable & Notes \\
\midrule
Gender & Derived & \texttt{ragender} & 1=male, 2=female (same coding) \\
Age\_New & Derived & \texttt{r8agey} & 1 if 45--59, 2 if $\geq 60$ \\
Marital & Derived & \texttt{r8mstat} & 1=married/partnered, 2=other \\
Education & Derived & \texttt{raedyrs\_e} & years $\rightarrow$ 3-level ordinal \\
Residence & Derived & \texttt{w8\_urindewr\_2011} & From rural-urban indicator file (EUL) \\
Hypertension & Self-report & \texttt{r8hibpe} & 1=ever diagnosed \\
Dyslipidemia & Self-report & \texttt{r8hchole} & 1=ever diagnosed (high cholesterol) \\
Diabetes & Self-report & \texttt{r8diabe} & 1=ever diagnosed \\
Cancer & Self-report & \texttt{r8cancre} & 1=ever diagnosed \\
CVD & Self-report & \texttt{r8hearte / r8stroke} & Composite (heart disease or stroke) \\
Smoke & Self-report & \texttt{r8smokev} & 1=ever smoked regularly \\
Drink & Self-report & \texttt{r8drink} & 1=drinks \\
BMI & Measured & \texttt{r8mbmi} & kg/m\textsuperscript{2}, nurse visit \\
BMI\_New & Derived & Derived from \texttt{r8mbmi} & Same 3-level categorisation \\
Biological Age & Computed & --- & Requires creatinine, BUN, platelets \\
wave / Time & Admin & --- & Not applicable cross-cohort \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection*{ELSA Statistical Analysis}
The same evaluation pipeline applied to the CHARLS holdout and HRS was applied to ELSA:
ROC-AUC, PR-AUC, Brier score, ECE, MCE, Cox logistic calibration slope and intercept, a decile
reliability diagram, and Vickers DCA in the 10\%--30\% threshold band. Two prediction variants
were evaluated: (1) \emph{raw} (uncalibrated) predictions from the CHARLS-trained Random Forest
applied directly to ELSA; and (2) \emph{isotonic-calibrated} predictions generated via a
split-calibrate-evaluate protocol, where $30\%$ of ELSA was randomly held out as a calibration
subset, an isotonic regression calibrator was fitted on those raw predictions, and the
calibrated probabilities were evaluated on the remaining $70\%$ evaluation subset. This
protocol prevents the calibrator from overfitting to the evaluation data. Both raw and isotonic
results are reported to provide a transparent picture of calibration behaviour.
...
% ============================================================
\subsection{Preliminary ELSA Replication Results}
% ============================================================

Table~\ref{tab:full-comparison} presents the performance of the CHARLS-trained model (re-fitted on
raw16) applied to the ELSA Wave 8 analytic sample ($n=8{,}416$) alongside CHARLS internal and HRS
results for a complete cross-cohort comparison.

\begin{table}[h!]
\centering
\small
\caption{Performance of the CHARLS-trained model (Random Forest + raw16) applied to HRS Wave 13
and ELSA Wave 8, compared with CHARLS internal results. HRS uses 12 of 17 raw17 features; ELSA
uses 14 of 17 (includes Residence and Dyslipidemia, absent in HRS). Biological Age is excluded
from both external cohorts. ELSA isotonic metrics are from the split-calibrate-evaluate protocol
(evaluated on the 70\% holdout). Calibration slope outside $[0.80,\,1.20]$ indicates systematic
miscalibration.}
...
HRS Wave 13 (raw16) & 20{,}605 & 33.9\% & 0.604 & 0.399 & 0.258 & 0.194 & 0.184 \\
\midrule
ELSA Wave 8 raw16 (raw) & 8{,}416  & 24.9\% & 0.604 & 0.297 & 0.201 & 0.097 & 0.271 \\
ELSA Wave 8 raw16 (iso)* & 5{,}892  & 24.9\% & 0.606 & 0.305 & 0.182 & 0.005 & 0.938 \\
\bottomrule
\end{tabular}

\vspace{4pt}
\footnotesize{*Isotonic metrics evaluated on the 70\% holdout subset ($n=5{,}892$) from the
split-calibrate-evaluate protocol; raw metrics use the full cohort.}
\end{table}

Discrimination for ELSA raw16 was $0.604$ (ROC-AUC) and $0.297$ (PR-AUC), nearly identical to
HRS (ROC-AUC $0.604$, PR-AUC $0.399$). The concordance in ROC-AUC between HRS and ELSA --
despite different continents, KOA proxy definitions, and ELSA having two additional features
(Residence and Dyslipidemia) -- suggests a stable raw16 transportability ceiling around AUROC
${\sim}0.60$ when Biological Age is absent. The lower PR-AUC in ELSA ($0.297$ vs.\ HRS $0.399$)
is partially attributable to ELSA's lower KOA proxy prevalence ($24.9\%$ vs.\ $33.9\%$), as
PR-AUC is prevalence-sensitive.

Calibration was severely degraded in the raw model (slope $0.271$, well outside $[0.80,\,1.20]$),
reflecting the prevalence mismatch between CHARLS ($13.3\%$) and the ELSA proxy ($24.9\%$): the
model, calibrated for a $13\%$ base rate, systematically under-predicts risk in the ELSA
population. After the split-calibrate-evaluate isotonic protocol, calibration was substantially
restored: ECE decreased from $0.097$ to $0.005$, slope normalised from $0.271$ to $0.938$, and
Brier improved from $0.201$ to $0.182$. DCA net benefit versus treat-all improved from $7.1$ to
$10.7$ avoided unnecessary interventions per 100 patients (relative reduction $9.5\% \rightarrow
14.2\%$). Crucially, discrimination did not meaningfully improve (ROC-AUC $0.604 \rightarrow
0.606$), confirming that calibration correction cannot rescue discriminative performance lost
due to feature impoverishment.

\textbf{Wave 9 sensitivity analysis.} A temporal sensitivity analysis using ELSA Wave 9
(2018--2019, $n=8{,}683$, KOA proxy prevalence $23.3\%$) produced consistent results:
ROC-AUC $0.596$, calibration slope $0.187$ (raw) and ROC-AUC $0.615$, slope $1.225$ (isotonic).
Wave 9 lacks a harmonized BMI variable (\texttt{r9mbmi}), so BMI was fully imputed from CHARLS
training data. The slightly lower raw discrimination ($0.596$ vs.\ $0.604$) is consistent with
the additional BMI missingness; the isotonic-calibrated discrimination ($0.615$) and calibration
restoration (ECE $0.009$) confirm the robustness of the split-calibrate-evaluate protocol.

\paragraph{Interpretation.} Four factors jointly explain the observed ELSA performance gap:
\begin{enumerate}
  \item \textbf{Missing Biological Age.} Biological Age is the dominant raw17 predictor (SHAP
        analysis, Step 04). Its exclusion from raw16 removes the strongest discriminative signal
        and accounts for the majority of the ROC-AUC loss in both ELSA and HRS.
  \item \textbf{Outcome-definition heterogeneity.} The ELSA KOA proxy uses ever-diagnosed
        arthritis (broader than CHARLS' knee-localised pain and HRS' current-wave arthritis).
        Outcome noise inflates apparent false-positive rates and degrades discrimination
        independently of feature quality.
  \item \textbf{Cross-cultural and demographic differences.} The ELSA sample is predominantly
        White British adults; CHARLS is a Chinese nationally-representative cohort. Differences
        in health behaviours, diagnostic ascertainment culture, comorbidity distributions, and
        healthcare access attenuate cross-cultural transportability.
  \item \textbf{Prevalence mismatch.} ELSA KOA proxy prevalence ($24.9\%$) is nearly double
        CHARLS ($13.3\%$), producing systematic under-prediction that manifests as poor
        calibration. Isotonic recalibration restores calibration (slope $0.271 \rightarrow
        0.938$) but cannot improve discrimination.
\end{enumerate}

Both the ELSA and HRS results do not constitute formal external validation; rather, they
represent a consistent conservative lower bound on transportability under maximum harmonisation
constraints (no Biological Age) and explicitly document the expected performance degradation
when the dominant predictor is absent. The Wave 9 sensitivity analysis supports temporal
...
\paragraph{HRS and ELSA replication and transportability.}
The HRS replication (ROC-AUC $0.604$; calibration slope $0.184$) and ELSA replication
(ROC-AUC $0.604$, calibration slope $0.271$) reveal consistent cross-cultural performance
attenuation. The near-identical ROC-AUC in two cohorts drawn from different continents (US
and UK), using different KOA proxy definitions (current-wave vs.\ ever-diagnosed arthritis),
and different available feature subsets (12 vs.\ 14 raw16 variables) supports a stable
transportability ceiling around AUROC ${\sim}0.60$ for raw16 without Biological Age. The four
factors identified in the Results -- missing Biological Age, outcome heterogeneity, cross-cultural
differences, and prevalence mismatch -- are hierarchically ordered: Biological Age alone is
expected to account for the majority of the observed ROC-AUC gap based on its SHAP importance
rank. Two downstream consequences follow. First, the HRS and ELSA results jointly establish a
\emph{consistent lower bound} on cross-cultural transportability under worst-case harmonisation;
actual performance with full raw17 (including KDM-computed Biological Age from complete
biomarker panels) is expected to be substantially higher. Second, the severe miscalibration
(slope $0.184$ HRS, $0.271$ ELSA) is mathematically predictable from the prevalence mismatches;
the split-calibrate-evaluate isotonic protocol in ELSA demonstrated that calibration can be
largely restored (ECE $0.005$, slope $0.938$) without changing the discriminative ceiling.

\paragraph{External validation and generalisability.}
The most critical gap separating this study from submission-strength clinical claims remains
the absence of full raw17 external validation in any cohort. We executed Stages A and B of the
planned two-stage protocol with the raw16 feature set under maximum harmonisation constraints.
Future work should pursue: (A) \emph{HRS full raw17 replication} by linking the HRS Venous Blood
Study biomarker file and computing KDM-based Biological Age; and (B) \emph{ELSA full-raw17}
pending access to linked NHS biochemistry records containing creatinine, BUN, and platelet
assays, or application of an alternative biological-clock formula (e.g., PhenoAge) using
available ELSA biomarkers. The Korean Longitudinal Study of Ageing (KLoSA) and WHO SAGE cohorts
are retained as secondary targets. Until a full raw17 external validation is completed,
clinical-utility language remains explicitly limited to internal-validation decision-support
signals.
...
\item \textbf{ELSA replication is preliminary and feature-incomplete.} The ELSA analysis uses raw16
(14 of 17 features), excludes Biological Age, and uses an ever-diagnosed arthritis proxy (broader
than CHARLS' current knee-localised pain). The ELSA public nurse visit files lack creatinine, BUN,
and platelet assays, preventing KDM-BA computation. BMI was fully imputed in the Wave 9 sensitivity
analysis, which may have attenuated performance.
...
Preliminary cross-cultural replications on two independent cohorts -- the US Health and Retirement
Study (HRS, Wave 13, $n=20{,}605$) and the English Longitudinal Study of Ageing (ELSA, Wave 8,
$n=8{,}416$) -- reveal consistent performance attenuation under maximum harmonisation constraints
(ROC-AUC ${\approx}0.60$ for both cohorts). This attenuation is largely explained by the
unavailability of Biological Age in both public files, the use of surrogate KOA proxy definitions,
prevalence mismatches, and genuine cross-cultural differences, rather than by a fundamental
failure of the raw17 feature set. Following a split-calibrate-evaluate isotonic recalibration
protocol, calibration was restored in ELSA (slope $0.271 \rightarrow 0.938$, ECE $0.097
\rightarrow 0.005$), but discriminative performance was not improved (ROC-AUC $0.604
\rightarrow 0.606$). A Wave 9 sensitivity analysis produced consistent results.

The findings establish a consistent conservative lower bound on model transportability without
Biological Age and motivate full raw17 external validation (with KDM-BA) in future work. Until
a full raw17 external validation is completed, clinical-utility claims remain bounded to the
...
Licence. The complete analysis pipeline code is available at the project repository. Reliability
diagrams and DCA curves for all cohorts are provided in the supplementary materials.
